...
<body>
    ...
    <div class="content">
        ...
        <div>
            <h2>Perkalian Dengan Bobot Per Row</h2>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        for (var i = 0; i < data.length; i++) {
            $('#nor' + i).html(kaliColumn(i + 1, "table_normalisasi"))
        }
    });
    ...
    function normalisasi(data, criteriaAll) {
        ...
        //     html += `<td>${criteriaAll[baris].code}</td>`;
        //     for (var i = 0; i < data.length; i++) {
        //         html += `<td>${Math.pow(data[i].value[baris],(criteriaAll[baris].bobot/100)).toFixed(2)}</td>`;
        //     }
        ...
        for (var baris = 0; baris < criteriaAll.length; baris++) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (criteriaAll[baris].type == "benefits") {
                    html += `<td>${Math.pow((data[i].value[baris]/getMax(b)),criteriaAll[baris].bobot/100).toFixed(2)}</td>`;
                } else if (criteriaAll[baris].type == "cost") {
                    html += `<td>${Math.pow((getMin(c)/data[i].value[baris]),criteriaAll[baris].bobot/100).toFixed(2)}</td>`;
                }
            }
            if (criteriaAll[baris].type == "benefits") {
                b++;
    ...
    function pembagian_hasil(data, criteriaAll) {
        ...
        // Header
        var sumTotalRow = []
        html += `<td>Total Bobot</td>`;
        for (var i = 0; i < data.length; i++) {
            sumTotalRow.push(kaliColumn(i + 1, "table_normalisasi"));
        }
        html += `<td>Hasil</td>`;
        for (var i = 0; i < data.length; i++) {
            html += `<tr>`;
            html += `<td> ${data[i].code} </td>`;
            html += `<td> ${kaliColumn(i + 1, "table_normalisasi")} </td>`;
            html += `<td> ${(kaliColumn(i + 1, "table_normalisasi")/sum(sumTotalRow)).toFixed(2)} </td>`;
            html += `</td>`;
            nilaiakhir.push((kaliColumn(i + 1, "table_normalisasi") / sum(sumTotalRow)).toFixed(2));
...
<script>
    function sumColumn(index, table) {
        var total = [];
    ...
    function kaliColumn(index, table) {
        var total = [];
        $('#' + table + ' tr').each(function() {
            if (!this.rowIndex) return;
            var customerId = $(this).find("td").eq(index).html();
        ...
        var res = total.map(function(v) {
            return parseFloat(v, 10);
        });
        let popped = res.pop();
        var array = ['1', '2', '3', '4'];
        var result = res.reduce(function(a, b) {
            return a * b;
        });
